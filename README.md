...
If you're deploying this application with [Coolify](https://coolify.io/) using raw compose deployment with Traefik as reverse proxy, the docker-compose.yml is already configured with the necessary labels and networks.

### Prerequisites

1. Coolify installed with Traefik configured
2. External networks created:
   - `coolify` network
   - Resource-specific network (auto-generated by Coolify based on `COOLIFY_RESOURCE_UUID`)
...
2. **Configure required environment variables in Coolify:**
   ...
   - `COOLIFY_RESOURCE_UUID` - Your Coolify resource UUID (auto-generated by Coolify)
   - `COOLIFY_APPLICATION_ID` - Your application ID in Coolify
   - `SERVICE_FQDN_ANKI` - Your domain name (e.g., `anki.yourdomain.com`)
   - `COOLIFY_CONTAINER_NAME` - Container name with timestamp (e.g., `anki-desktop-uuid-timestamp`)

3. **Deploy via Coolify:**
   - Use "Raw Compose" deployment type
   - Paste the docker-compose.yml content
   - Coolify will automatically inject environment variables
   - The application will be accessible via HTTPS at your configured domain
...
- **AnkiConnect API**: Can be optionally exposed (see docker-compose.yml comments)

### Accessing AnkiConnect API

By default, AnkiConnect (port 8765) is available within the Docker network but not exposed externally. If you need external access:

1. Uncomment the AnkiConnect Traefik labels in `docker-compose.yml`
2. Set `SERVICE_FQDN_ANKICONNECT` environment variable (e.g., `ankiconnect.yourdomain.com`)
3. Redeploy the application

### Network Configuration

The application connects to two networks:
- `coolify` - Main Coolify network for Traefik routing
- `${COOLIFY_RESOURCE_UUID}` - Resource-specific isolated network
